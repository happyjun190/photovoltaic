...
public class RedisConstants {
	...
	public enum Prefix {
		...
		/** 配送版的token */
		LS_TOKEN("token.to.userid.industrynet-ls:"),
		
		/** 配送版的userInfo */
		LS_USER_INFO("user.info.industrynet-ls:", KeyType.hash),
		
		/** 手机验证码 */
		PHONE_AUTHCODE("identifyCode:", TimeInSecond._10_MINS),
		
		/** 帮助问题点击量 */
		HELP_CLICK("industrynet.helpclickset", KeyType.hash, TimeInSecond.INFINITE),
		
		/** 微信支付二维码CODEURL */
		WECHATPAY_CODEURL("web.nativewechat.codeurl.orderid:", TimeInSecond._120_MINS),
		
		/** 用户访问记录，防止频繁请求 industrynet.req:<userId>.<method> */
		REQUEST("industrynet.req:", TimeInSecond.NA),
		
		/** 连锁采购转账订单，push进去给PHP pop出来统一转账 */
		CHAIN_ORDER_YIBAO("industrynet.admin.queue:chainyibao", KeyType.list, TimeInSecond.NA),
		
		/**某药店在某个限量特惠活动期间采购量，冒号后面跟着wholesaleId */
		DRUGSTOREAMOUNT_LIMITEDDEALS("{industrynet.limiteddeals.}drugstoreamount:", KeyType.zset, TimeInSecond.NA),
		
		/** 访问限量特惠下单 用户记录，防止同一时间并发量过大，冒号后面跟着wholesaleId */
		USER_REQ_AMOUNT_LIMITEDDEALS("industrynet.limiteddeals.userreqamount:", TimeInSecond.NA),
		
		/** 访问限量特惠下单数 也就是库存,冒号后面跟着wholesaleId */
		ORDERAMOUNT_LIMITEDDEALS("{industrynet.limiteddeals.}orderamount:", TimeInSecond.NA),
		
		/** 某个用户的购物车数量，冒号后面跟着userId*/
		USER_BUYCAR_AMOUNT("industrynet.buycar.mount:", TimeInSecond.NA),
		
		// === 分割线， 以下这些即将被废弃。新的请定义在上面，即将被废弃的请移到下面来： === 
		
		/** 店员版用户经度，有序集合： zadd(GPS_LNG, userId, longitude); */
		//GPS_LNG("industrynet.gps.lng", KeyType.zset, TimeInSecond.NA),
		/** 店员版用户纬度，有序集合： zadd(GPS_LAT, userId, latitude); */
		//GPS_LAT("industrynet.gps.lat", KeyType.zset, TimeInSecond.NA),
	...
	//public static final String USERTOKENHEAD_YSB_LS = "token.to.userid.industrynet-ls:";// 店员版配送工具
	 ...
	 * 记录ysbuserinfo信息
	 */
	//public static final String USER_INFO_YSB = "user.info:";// hash缓存的缓存key值user.info.industrynet:#{userId}
	 ...
	 */
	//public static final String USER_INFO_YSB_LS = "user.info.industrynet-ls:";// hash缓存的缓存key值user.info.industrynet-ls:#{userId}
	 ...
	 * 店员版用户经度
	 */
	//public static final String YSB_GPS_LNG = "industrynet.gps.lng";
	/**
	 * 店员版用户纬度
	 */
	//public static final String YSB_GPS_LAT = "industrynet.gps.lat";
	...
	/** 用户访问记录，防止频繁请求 industrynet.req:<userId>.<method> */
	//public static final String YSB_REQUEST = "industrynet.req:";
	
	//public static final String YSB_CHAINYIBAO = "industrynet.admin.queue:chainyibao";
	...
	//public static final String CAPTCHA = "industrynet.captcha:";
	/** 验证码，field，验证码内容*/
	...
	//public static final String WEB_TOKEN_PREFIX = "industrynet.web.token:";
...
//	public static final String ADMIN_TOKEN_PREFIX = "industrynet.admin.token:";
	...
	//public static final String HELPCLICKSET = "industrynet.helpclickset";
